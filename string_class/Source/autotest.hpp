...
private:
    bool doOCFTests(std::ostream &anOutput) {
...

        {
            ...
            // }
            // std::cout<<"\n";
            // std::cout<<theSTDString7<<"\n";
            // std::cout<<theSTDString7.size()<<" "<<theECEString7.size() <<"\n";
            // std::cout<<std::strlen(theECEString7.getBuffer())<<"\n";
            // std::cout<<theECEString7.getBuffer()[theECEString7.size()-1]<<"\n";
            // std::cout<<theECEString7.getBuffer()[theECEString7.size()]<<"\n";
    ...
    bool doInsertTests(std::ostream &anOutput) {
        ...
        theTracker.enable(true).reset();
        {
            ...
            if (theSTDString1.compare(theECEString1.getBuffer()) != 0) {
                anOutput << "1-insert(size_t anIndex, const char* aBuffer, size_t aStrIndex, size_t aStrCount) failed\n";
                return false;
            }
            ...
            if (theSTDString1.compare(theECEString1.getBuffer()) != 0) {
                anOutput << "2-insert(size_t anIndex, const char* aBuffer, size_t aStrIndex, size_t aStrCount) failed 2\n";
                return false;
            }
        }
        theTracker.reportLeaks(anOutput);
    ...
    bool doAppendTests(std::ostream &anOutput) {
        ...
        theTracker.enable(true).reset();
        {
            ...
            // for(int i=0; i<theECEString1.size(); i++){
            //     std::cout<<theECEString1[i];
            // }
            // std::cout<<"\n";
            // std::cout<<theSTDString1<<"\n";
    ...
    bool doReplaceTests(std::ostream &anOutput) {
        ...
        theTracker.enable(true).reset();
        {
            ...
            if (!(ECEString5.getBuffer() == STDString1)) {
                anOutput << "8-replace(size_t pos, size_t len, const String& aString)\n";
                return false;
            }
        }
        theTracker.reportLeaks(anOutput);
